...
const postUserDB = async (response, request) => {
    console.log(request)
    const {id, link, typeOfAssignment} = request;
    // console.log("HERE IS REQUEST: ", request)
    // const {id, linkedInLink} = request;
    const params = {
      TableName: "TheDeckUser",
        Key: {
            userId: id
        }
    };
...
exports.handler = async (event) => {
  ...
  const response = {
    statusCode: 200,
    body: JSON.stringify("Successfully added linkedin"),
    headers: {
  ...
  // if (event.path == "/myLinkedInFunction") {
  //     await postUserDB(response, event);
  // }
  await postUserDB(response, event);
