...
export default function BankingWidgetSummary({ title, total, icon, percent, color = 'primary', chartData }) {
  ...
  return (
    ...
    >
      ...
      >
        <CustomIcon icon={icon} width={24} height={24} />
      </IconWrapperStyle>

      <Stack spacing={1} sx={{ p: 3 }}>
        <Typography sx={{ typography: 'subtitle2' }}>{title}</Typography>
        <Typography sx={{ typography: 'h3' }}>{formatCurrency(total)}</Typography>
        <Stack direction="row" alignItems="center" flexWrap="wrap">
          <CustomIcon width={20} height={20} icon={percent >= 0 ? 'eva:trending-up-fill' : 'eva:trending-down-fill'} />
